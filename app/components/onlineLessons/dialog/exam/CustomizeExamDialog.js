...
class CustomizeExamDialog extends Component {
    ...
    removeRow = (row) => () => {
        ...
        if (index > -1) {
            ...
            rows.forEach((item, count) => {
                resetSpecificField(dispatch, DIALOG.CUSTOMIZE_EXAM, `${item}.content_${count}`, item.content);
            });
            this.setState({rows});
        }
    }
    ...
    submit = (values) => {
        ...
        const {actions: {createCustomizeExam}, courseId, pointId} = this.props;
        ...
        return createCustomizeExam(courseId, pointId, params)
            .then(() => {
