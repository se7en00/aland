...
const mapStateToProp = (state) => {
    ...
    return {
        ...
        categoryList: state.point?.categoryList
    };
};

@connect(mapStateToProp)
@reduxForm({form: DIALOG.TRAINING_CUSTOMIZE_EXAM})
class CustomizeExamDialog extends Component {
    static dialogName = DIALOG.TRAINING_CUSTOMIZE_EXAM;
    ...
    closeDialog = () => {
        this.props.dispatch(clearSubmitErrors(DIALOG.TRAINING_CUSTOMIZE_EXAM));
        this.props.dispatch(reset(DIALOG.TRAINING_CUSTOMIZE_EXAM));
        this.props.hideDialog(DIALOG.TRAINING_CUSTOMIZE_EXAM)();
    ...
    removeRow = (row) => () => {
        ...
        if (index > -1) {
            ...
            rows.forEach((item, count) => {
                resetSpecificField(dispatch, DIALOG.TRAINING_CUSTOMIZE_EXAM, `${item}.content_${count}`, item.content);
            });
            this.setState({rows});
        }
    }
    ...
    submit = (values) => {
        ...
        const {actions: {createCustomizeExam}, trainingId} = this.props;
        ...
        return createCustomizeExam(trainingId, params)
            .then(() => {
        ...
        return (
            <Modal
                ...
                footer={[
                    <Button
                        key="submit"
                        onClick={() => dispatch(submit(DIALOG.TRAINING_CUSTOMIZE_EXAM))}
                        loading={submitting}
                        type="primary"
                    >
                        保存
